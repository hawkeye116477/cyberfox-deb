...
if [ ! -d "$Dir/tmp/kyberfoxhelper-$VERSION" ]; then 
    mkdir $Dir/tmp/kyberfoxhelper-$VERSION
fi
...
# Copy deb templates
if [ -d "$Dir/kyberfoxhelper/debian" ]; then
	cp -r $Dir/kyberfoxhelper/debian/ $Dir/tmp/kyberfoxhelper-$VERSION/
else
...
cd ~/git/cyberfox-deb/BUILD/tmp/kyberfoxhelper-$VERSION
debuild -us -uc -d
